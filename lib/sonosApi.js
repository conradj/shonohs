...
export default class {
  ...
  async setTokenOnCallback(code, callbackUri) {
    const options = {
      code,
      redirect_uri: callbackUri, //`${process.env.DOMAIN}/sonosAuthCallback`,
  ...
  async refreshToken(accessTokenId) {
    ...
    if (accessToken.expired(300)) {
      ...
      try {
        ...
        console.errpr("Error refreshing access token: ", error.message);
        return null;
      }
    }
